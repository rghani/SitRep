...
namespace Microsoft.Samples.Kinect.BodyBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        Dictionary<string,Gesture> workoutGestureList = new Dictionary<string,Gesture>();
        ...
        async Task<bool> loginAsyc()
        {
            ...
            //At this point, wait until loginTask is complete
            bool success = await loginTask;
            return success;
        }

        async Task<ParticleDevice> getDeviceAsync()
        {
            Task<ParticleDevice> retrieveDeviceByID = ParticleCloud.SharedCloud.GetDeviceAsync(ProtectedInfo.deviceID);
            return await retrieveDeviceByID;
            ...
            if (dataReceived)
            {
                using (DrawingContext dc = this.drawingGroup.Open())
                {
                    ...
                    foreach (Body body in this.bodies)
                    {
                        ...
                        {
                            track_id = body.TrackingId;
                            this.DrawClippedEdges(body, dc);
                            if (!isBodyTracked)
                            {
                                isBodyTracked = true;

                                VGBFrameSource = new VisualGestureBuilderFrameSource(this.kinectSensor, 0);
                                this.VGBFrameReader = this.VGBFrameSource.OpenReader();
                                workoutController = new WorkoutController();
                                //Iterates over all visual gesture databases
                                foreach (var db in workoutDatabases.Values)
                                {
                                    VGBFrameSource.AddGestures(db.AvailableGestures);
                                    //Iterates and adds all available gesture in a database
                                    foreach (var gesture in db.AvailableGestures)
                                    {
                                        workoutGestureList.Add(gesture.Name, gesture);
                                        if (VGBFrameSource.GetIsEnabled(gesture) == false)
                                            VGBFrameSource.SetIsEnabled(gesture, true);
                                    }
                                }
                                ...
                                {
                                    this.VGBFrameReader.FrameArrived += this.VGB_FrameArrived;
                                }
                                if (workoutController != null)
                                {
                                    workoutController.OnWorkoutDetected += WorkoutDetected;
                                }
        ...
        void VGB_FrameArrived(object sender, VisualGestureBuilderFrameArrivedEventArgs args)
        {
            using (var frame = args.FrameReference.AcquireFrame())
            {
                if (frame != null && frame.DiscreteGestureResults != null && frame.ContinuousGestureResults != null)
                {
                    var discreteResult = frame.DiscreteGestureResults[workoutGestureList["Shoulder_Press"]];
                    var continousResult = frame.ContinuousGestureResults[workoutGestureList["Shoulder_PressProgress"]];
                    workoutController.getResults(discreteResult, continousResult,workoutGestureList["Shoulder_Press"].Name);
                    workoutController.checkStatus();
                }
            }
        }
